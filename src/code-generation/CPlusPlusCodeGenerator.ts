...
export default class CPlusPlusCodeGenerator extends CodeGeneratorBase<CPlusPlusCodeGeneratorSettings> {
  ...
  addNative(native: CodeGenNative): this {
    ...
    const params = native.params.map(({
      type,
      name
    }) => `${this.formatType(type)} ${name}`).join(', ')
  ...
  sol2Bindings() {
    ...
    this.natives.forEach((v, k) => {
      ...
      v.forEach(native => {
        const name = this.settings.cppCompliant ? this.transformNativeName(native.name) : native.name
        const nativebinding = `${k}_table.set_function("${name}", ${k}::${name});`
        writer.writeLine(nativebinding)
      })
